...
public final class BluetoothPairingDialog extends AlertActivity implements
    ...
    private void onCancel() {
		...
		//Pairing Dialog has been cancelled
		LocalBluetoothManager manager = LocalBluetoothManager.getInstance(this);
		if (manager != null) {		
			CachedBluetoothDeviceManager deviceManager = manager.getCachedDeviceManager();
            CachedBluetoothDevice cachedDevice = deviceManager.findDevice(mDevice);

			if (cachedDevice != null) {
				cachedDevice.setPairingDialogCancelled(true);
				cachedDevice.refresh();
				Log.e(TAG, "Pairing Dialog has been cancelled,set Cancelled as true");
			}				
		}
