...
public final class BluetoothDevicePreference extends Preference implements
    ...
    public void onDeviceAttributesChanged() {
		...
		if (mCachedDevice.getBondState() != BluetoothDevice.BOND_BONDING) {		
			mCachedDevice.setPairingDialogCancelled(false);
		}
        ...
        boolean enable = !mCachedDevice.isBusy();
		if ((mCachedDevice.getBondState() == BluetoothDevice.BOND_BONDING) && 
			(mCachedDevice.getPairingDialogCancelled() == true) && 
			(summaryResId == 0)) {
			enable = true;
		}
        setEnabled(enable);
    ...
    private int getConnectionSummary() {
        ...
        switch (cachedDevice.getBondState()) {
            case BluetoothDevice.BOND_BONDING:
				 if (cachedDevice.getPairingDialogCancelled() == false) {
